...
- The second column should have the header `Value` and include the metadata values: `EPAair_O3_PM25_NC1819_Processed.csv`,  
`2018-2019`, and `EPA Air Quality System (AQS)`.
...
```{r project.setup, results='hide', message=FALSE}
...
summary_PM25 <- epa_data %>% 
  ...
  arrange(mean_PM2.5) %>%
  rename(County = COUNTY, `µg/m³` = mean_PM2.5)
...
summary_Ozone <- epa_data %>% 
  ...
  arrange(mean_Ozone) %>%
  rename(County = COUNTY, `µg/m³` = mean_Ozone)
...
```{r echo=FALSE}
# Create the first table, trying to get ride of auto-labeling by setting 'null"
## ???

kable(summary_PM25, caption = "Table 1: Mean Particulates (2.5mm)", 
      label = NULL)
# Create the second table
kable(summary_Ozone, caption = "Table 2: Mean Ozone", 
      label = NULL)
```
...
```{r, echo=FALSE, fig.cap="Distribution of Ozone Levels by Month 2018", fig.height=4, fig.width=6}
library(ggplot2)
...
data_2018$MonthName <- month(data_2018$Date, label = TRUE, abbr = FALSE)

# Create boxplot for 2018
p1 <- ggplot(data_2018, aes(x = MonthName, y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Levels (µg/m³)", 
...
```{r, echo=FALSE, fig.cap="Distribution of Ozone Levels by Month 2019", fig.height=4,fig.width=6}
...
# Create a new column for month names
data_2019$MonthName <- month(data_2019$Date, label = TRUE, abbr = FALSE)

# Create boxplot for 2019
p2 <- ggplot(data_2019, aes(x = MonthName, y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Levels (µg/m³)", 
       title = "Ozone Levels in 2019") + theme(
    plot.title = element_text(hjust = 0),
    axis.text.x = element_text(angle = 45, hjust = 1)  # Tilt x-axis labels
  )
print(p2)
...
## Git Repository
https://github.com/RuoxiWuRosie/EDE_Fall2024 
