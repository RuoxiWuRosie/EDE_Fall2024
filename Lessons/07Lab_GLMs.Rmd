...
#Exercise 1:
 plot_1<- 
  ...
  labs(x="Ozone AQI calue", y= "Density", fill="")
print(plot_1) 

# plot 2
boxplot_03 <- ggplot(EPAair, aes(x = as.factor(Year), y=Ozone))+ 
  geom_boxplot()+
  scale_fill_manual(values = c("darkgreen","darkblue"))+
  labs(x="Year", y= "Ozone Density", fill="")
print(boxplot_03) 

# violin plot (check key!!)
```
...
particulates.regression <- 
  lm(EPAair$Ozone ~ 
       EPAair$PM2.5)
summary(particulates.regression)
...
EPAair_corr <- 
  ...
  select(Ozone:PM2.5) %>%
  na.omit()
Ozone_PM2.5_corr <- cor(EPAair_corr)
corrplot(Ozone_PM2.5_corr, upper = "ellipse")

# OzoneAll<- lm(data = EPAair_corr, Ozone_axi
