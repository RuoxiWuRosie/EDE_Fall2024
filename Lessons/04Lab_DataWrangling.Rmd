...
NTL.phys.data.PeterPaul <- 
  # %in%  find or skip it
  "Peter Lake"filter(NTL.phys.data, lakename %in% c("Paul Lake", ))
...
NTL.phys.data.9099 <-
  filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999))
NTL.phys.data.9099

```
Question: Why don't we filter using row numbers?

> Answer: it's not as precise, and the row numbers are not under our control. We need to filter on the data itself.
...
NTL.phys.data.processed3 <-
  ...
  filter(month(sampledate) == 7) # the month can be called from sampledate, because of the function of lubridate?
